...
void initCorePeripherals(void)
{
    ...
    /* explicitly set priority grouping to PRE4_SUB0 (4 bits preemption, 0 sub),
       so NVIC_SetPriority values map directly to preemption priority, matching
       E230 semantics. must be called before the first nvic_irq_enable(). */
...
void COMP_Init(void)
{
    ...
    cmp_mode_init(CMP0, CMP_MODE_HIGHSPEED, CMP_INVERTING_INPUT_PA5, CMP_HYSTERESIS_NO);
...
void initAfterJump()
{
    __enable_irq();
    fmc_wscnt_set(2);
    // F3x0 series uses fmc_wait_state_enable() instead of fmc_prefetch_enable()
